...
class Timbre(object):
    ...
    def __init__(self, f_0, n_partials,octaves = 1):
    ...
    def disMeasure(self, octaves = 1):
        ...
        norm = [diss/max(dissonances) for diss in dissonances] # normalize
        self.dissonances = dissonances # add raw values to self
        self.norm = norm # add normalized values to self

    def disPlot(self):
        """Plots relative dissonance at all intervals for a given tone"""
        try:
            plt.plot(np.arange(1,self.octaves, self.increment),self.norm)
            ...
            if hasattr(self, 'attenuation'):
                plt.suptitle('%s Hz Fundamental, %d Partials, AttenRate %.2f, %s' % (self.f_0, self.n_partials, self.attenuation, self.harmonics), fontsize = 12)
            else:
                plt.suptitle('%s Hz Fundamental, %d Partials, %s' % (self.f_0, self.n_partials,self.harmonics), fontsize = 12)
    ...
    def wavePlot(self):
        ...
        plt.plot(self.data, label = '%s harmonics\nf_0 = %d Hz\n%d partials' % (self.name, self.f_0, self.n_partials))
    ...
    def timbreGen(self, length = 5, writeFile = False, envelope = False):
        """Generates audio data and optionally a .wav file of a timbre. 
        ...
        if writeFile:
            write('Timbre/audio/%dHz_%dpartials_%s.wav' % (self.f_0, self.n_partials, self.harmonics ), 44100.0, self.wavData)

    def timbreSweep(self, length = 90, writeFile = False):
        """Generates audio data and optionally a .wav file of a timbre swept against itself. 
        
               * Default length 30 seconds. 
               * files in Timbre/audio/
        """
        sampleInc = 1.0/44100
        f_Start, f_Stop = self.freqs, [2.3*f for f in self.freqs] # 2 arrays, one of initial and one of final partials 
        ...
        if writeFile:
            write('Timbre/audio/%dHz_%dSec_OctSweep%dpartials_%s.wav' % (self.f_0,length, self.n_partials, self.harmonics ), 44100.0, sweepData)
    ...
    def consFreqs(self, makePlot = False):
        """Optionally plots a bar graph showing the peak dissonant an consonant frequencies for a given Timbre object"""
        x = np.asarray(self.norm)
        con = argrelextrema(x, np.less)
        self.consonances = [self.f_0*(1 + c*self.increment) for c in con]
        self.consonances = self.consonances[0]
    ...
    def writeChord(self,chordNotes = [0,4,7], length = 5, verbose = False):
        ...
        """
        consRatios = [1]
        if hasattr(self,'consonances'):
            consRatios[1:] = self.consonances/self.f_0 # consonance ratios        
        ...
        wavData = wavData/mx
        wavData = np.asarray(32000*wavData, dtype = np.int16)
        write('/audio/%dHz_Chord.wav' % self.f_0, 44100.0, wavData)

#####################################
# audio file writing is not working #
#####################################
...
if __name__ == '__main__':
    ...
    # foo = Custom(f,a)
    # print 'len(f) = ', len(f)
    foo = Odd(n_partials = 9, attenuation = .8)
    # # foo.timbreSweep(length = 60)
    foo.disMeasure(octaves = 1)
    # # foo.timbreGen()
    # # foo.consDisFreqs()
    # foo.consFreqs()
    # foo.writeChord(chordNotes = [0,5], length = 5, verbose = True)
    # # foo.partialsPlot()
    # foo.wavePlot()
    foo.disPlot()
    ...
    # bar = even(n_partials = 10)
    # bar.dismeasure(octaves = 2)
    # # bar.timbreGen()
    # #
    # baz = even(n_partials = 20)
    # baz.dismeasure(octaves = 2)
    # baz.timbreGen()
    # moo = sawtooth(n_partials = 7)
