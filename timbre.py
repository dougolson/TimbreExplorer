"""
Timbre Module contains base class and methods
"""
...
class Timbre(object):
    """Parent class for the various timbre objects. Use subclasses Even, Odd,
    Evenodd, Square, Sawtooth, Triangle, Beam and Custom to create objects
    """
    ...
    def disMeasure(self, octaves=1):
        """Calculates the relative dissonance for a given Timbre object at
        all intervals within a specified range. disMeasure(octaves = 1).

        returns an an array of relative dissonances **self.dissonances** and
        a normalized dissonance array **self.norm**
        ...
        for i in np.arange(1, self.octaves, inc):
            x2 = [freq * i for freq in
                  frequencies]  # iterate each freq in the frequency array over
                   # x increments
            x2 = zip(x2, amplitudes)  # zip each frequency with its associated
            # amplitude
            diss_j = []  # empty array for j loop
            for j in x1:
                diss_k = []  # empty array for k loop
                for k in x2:
                    # perform calc on each element of x2,
                    # append result to diss_k array
                    S = Dstar / (S1 * min(j[0], k[0]) + S2)
                    Fdif = abs(k[0] - j[0])
                    a = min(j[1], k[1])
                    diss_k.append(
                        a * (C1 * np.e ** (A1 * S * Fdif) + C2 * np.e ** (
                            A2 * S * Fdif)))

                diss_j.append(np.sum(diss_k))  # sum each k iteration and
                # append to j array
            dissonances.append(np.sum(diss_j))  # sum each j iteration, append
            # to dissonances array
        normalizedDissonances = [diss / max(dissonances) for diss in
                                 dissonances]  # normalize
        self.dissonances = dissonances  # add raw values to self
        self.normalizedDissonances = normalizedDissonances
        # add normalized values to self
    ...
    def disPlot(self, normalized=True):
        ...
        labels = [str(n) for n in
                  range(len(np.arange(0, self.width + 1. / 12, 1. / 12)))]
        plt.xticks([2 ** i for i in
                    np.arange(0, self.width + 1. / 12, 1. / 12)], labels)
                   # set ticks at
                   # equal tempered intervals
        if hasattr(self, 'attenuation'):
            plt.suptitle('%s Hz Fundamental, %d Partials, AttenRate %.2f, %s' %
                         (self.f_0, self.numPartials, self.attenuation,
                          self.harmonics), fontsize=12)
        else:
            plt.suptitle('%s Hz Fundamental, %d Partials, %s' %
                         (self.f_0, self.numPartials, self.harmonics),
                         fontsize=12)
    ...
    def partialsPlot(self):
        """Plots the partials and their relative amplitudes for the timbre being
        examined. Positive Amplitudes in red, Negative amplitudes in blue.
        """
        wdth = self.f_0 / 4.0
    ...
    def wavePlot(self):
        ...
        plt.plot(self.data, label='%s harmonics\nf_0 = %d Hz\n%d partials'
                 % (self.name, self.f_0, self.numPartials))

        plt.axis([0, 100, -1.1, 1.1])
        labels = [str(round(1000 * n, 2)) for n in
                  np.arange(0, T * 1.01, T / 10.0)]
    ...
    def consDisFreqs(self, makePlot=False):
        """
        Generates a list of the consonances (minima on the dissonance plot)
        for a given timbre. The consonances are used by consFreqs and
        ...
        if makePlot:
            ...
            plt.bar(np.round(self.consonances, 2), hCon,
                    max(ticks) / 200.0, color='b',
                    align='center', label='Consonant')

            plt.bar(np.round(self.dissonances, 2), hDis,
                    max(ticks) / 200.0, color='r',
                    align='center', label='Dissonant')
    ...
    def consFreqs(self, makePlot=False):
        ...
        if makePlot:
            ...
            plt.bar(np.round(self.consonances, 2), hCon,
                    max(ticks) / 500.0, color='b', align='center',
                    label='Consonances for %d Hz Fundamental' % self.f_0)
    ...
    def timbreGen(self):
        ...
        write('TimbreAudio/%dHz_%dpartials_%s.\
wav' % (self.f_0, self.numPartials, self.harmonics), 44100.0, wavData)

        print "File written to TimbreAudio folder"

    def timbreSweep(self):
        """Generates audio data and a .wav file of a timbre swept against
        itself, i.e. one tone is held constant, the other ascends for just over
        1 octave.
               * prompts user for file length
               * files in TimbreAudio in cwd
        """
        ...
        while True:
            ...
            try:
                ...
                print "Value must be an integer"
        sampleInc = 1.0 / 44100
        f_Start, f_Stop = self.freqs, [2.2 * f for f in self.freqs]
        # 2 arrays, one of initial and one of final partials
        times = np.arange(0, length, sampleInc)  # array of chosen length at
        # 44.1 kHz sample rate
        ramp = 1. * times / length  # linear ramp
        rampArray = [f0 * (1 - ramp) + f1 * ramp for f0, f1 in
                     zip(f_Start, f_Stop)]  # generates an list of arrays, one
                     # for each frequency
        sweepData = np.zeros(len(times))

        phaseCorrect = [
            np.add.accumulate(times * np.concatenate((np.zeros(1), 2 * np.pi *
                                                      (x[:-1] - x[1:]))))
            for x in rampArray]
        ...
        for array, amp, phase in zip(rampArray, self.amps, phaseCorrect):
            #swept frequency
        ...
        write('TimbreAudio/%dHz_%dSec_OctSweep%dpartials\
        _%s.wav' % (self.f_0, length, self.numPartials,
                    self.harmonics), 44100.0, sweepData)

        print "File written to TimbreAudio folder"

    def writeConsonantChord(self):
        """
        Writes a chord with the selected half steps at the timbre's nearest
        consonant pitches.
        ...
        EqTemp_Ratios = [(2 ** (1. / 12)) ** x for x in range(13)]  # array of
        # EQ Temp ratios
        mins = []
        for i in range(len(consRatios)):  # for each consonant frequency
            ...
            for j in range(len(EqTemp_Ratios)):  # for each 12TET step
                tmp.append(
                    abs(consRatios[i] - EqTemp_Ratios[j]))  # append tmp with
                    #the difference between ConsFreq and Step
            if tmp.index(min(tmp)) not in mins:  # if the index isn't already
            # in mins
                mins.append(tmp.index(min(tmp)))# add the min value for each i
                # pass to mins array
            else:
                mins.append(
                    tmp.index(min(tmp)) + 1)
                     # if it already is in mins, add one (i.e. make it the next
                     # half step up)
                     # mins contains the indeces of the consFreqs, which
                     # corresponds
                     # to the interval, i.e. 3 is a minor 3rd, 7 is a 5th, etc
...
Press Enter twice when done: " % ', '.join([str(m) for m in mins]))
        ...
        for freq, amp in zip(self.freqs, self.amps):
            for chord in chordNotes:
                y = amp * np.sin(2 * np.pi * noteDict[chord] * freq * x)
                wavData = np.sum([wavData, y], axis=0)
        mx = 1.1 * (max(abs(wavData)))
        wavData = np.asarray(wavData)
        wavData = wavData / mx
        ...
        write('TimbreAudio/%dHzFund_%s_%sConsonantChord.wav' % (
            self.f_0, '-'.join([str(note) for note in sorted(chordNotes)]),
            self.harmonics), 44100.0, wavData)
    ...
    def writeEqTempChord(self):  # , chordNotes = [0, 4, 7]
        ...
        chordNotes = []
        while True:
            noteChoice = raw_input(
                "Enter the half step values you want; \
        ...
        write('TimbreAudio/%dHzFund_%s_%s_EqTemp_Chord.wav' % (
            self.f_0, '-'.join([str(note) for note in sorted(chordNotes)]),
            self.harmonics), 44100.0, wavData)
        print "File written to TimbreAudio folder"
...
class InputError(Error):
    """
    For user input error 'value not in list'
    """
    pass
...
# if __name__ == '__main__':
    ...
    # evenTest = Timbre.Even()
    # oddTest = Timbre.Odd()
    # evenOddTest = Timbre.Evenodd()
    # squareTest = Timbre.square()
    # triangleTest = Timbre.triangle()
    # sawtoothTest = Timbre.sawtooth()
    # beamTest = Timbre.Beam()

    # f = [260.44805563090279, 520.35237836904787, 778.62550242892019, 1039.6172909525806,
    # 1300.609079476241, 1558.3384706433555, 1819.330259167016, 2078.6908490124033]
    # a = [0.16724917882935164, 1.0, 0.05825872786812774, 0.019126990466560075,
    # 0.097543923859240947, 0.014853679015129385, 0.013322988640851378, 0.012819862560537666]
    ...
    # f, a = fft6.get_fft_freq_array('./samples/Bassoon.ff.C4.wav', mkPlot=False)
    # print f
    # print a
